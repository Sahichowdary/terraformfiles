resource "aws_security_group" "bastion_sg-poc" {
  ...
  ingress {
    ...
    to_port     = 3306
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # Allowing MySQL access from anywhere, you may restrict it as per your requirements
  }
...
resource "aws_instance" "bastion_host-POC" {
  ...
  security_groups = [aws_security_group.bastion_sg-poc.id]
  key_name        = "aws-poc-demo"
